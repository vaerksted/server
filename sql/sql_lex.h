...
public:
  ...
  */
  ...
  bool add_alter_list_item_convert_to_charset(const Charset_collation_map_st &map,
                                              CHARSET_INFO *cs)
  {
    if (create_info.add_table_option_convert_charset(map, cs))
      return true;
    alter_info.flags|= ALTER_CONVERT_TO;
    return false;
  }
  bool
  add_alter_list_item_convert_to_charset(const Charset_collation_map_st &map,
                                         CHARSET_INFO *cs,
                                         const Lex_extended_collation_st &cl)
  {
    if (create_info.add_table_option_convert_charset(map, cs) ||
        create_info.add_table_option_convert_collation(map, cl))
      return true;
    alter_info.flags|= ALTER_CONVERT_TO;
    return false;
  }
