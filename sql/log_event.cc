...
code_name(int code)
{
  ...
  case Q_COLLATIONS_SESSION: return "Q_COLLATIONS_SESSION";
  }
...
Query_log_event::Query_log_event(const uchar *buf, uint event_len,
  ...
  :Log_event(buf, description_event), data_buf(0), query(NullS),
   ...
   character_set_collations_str({0,0}), flags2(0),
  ...
  */
  ...
  for (const Log_event::Byte* pos= start; pos < end;)
  {
    ...
    case Q_COLLATIONS_SESSION:
    {
      ...
      pos+= count * 4;
      character_set_collations_str= Lex_cstring((const char *) pos0,
                                                (const char *) pos);
      break;
    }
